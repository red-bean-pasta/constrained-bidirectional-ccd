...
func _init(
	...
	max_rad = p_max_rad
	rest_rad = p_rest_rad
	Utils.assert_range(rest_rad, min_rad, max_rad)
...
func clamp(rad: float) -> float:
	return Utils.wrap_clamp_rad(rad, min_rad, max_rad)
	
func check_within_range(rad: float) -> bool:
	var wrapped := Utils.wrap_rad(rad, min_rad, max_rad)
